"""""""""
Problem: How to schedule workers in multiple daily shifts so that each shift is staffed by enough workers to maintain 
operations. A hospital supervisor needs to create a weekly schedule for four nurses, subject to the following conditions

1) Each day is divided into three 8-hour shifts.
2) On each day, all nurses are assigned to different shifts and one nurse has the day off.
3) Each nurse works five or six days a week.
4) No shift is staffed by more than two different nurses in a week.
5) If a nurse works shifts 2 or 3 on a given day, he must also work the same shift either the previous day or the 
    following day.
...
Assign nurses to shifts: The table below shows one possible assignment of the nurses, who are labeled A, B, C, D, to 
shifts, numbered 0 - 3 (where 0 means that the nurse does not work on that day).
...
Assign shifts to nurses: The following table shows the corresponding assignment of shifts to nurses.
