...
columns = ['Commodity',	'Unit',	'1939 price (cents)','Calories','Protein (g)','Calcium (g)','Iron (mg)','Vitamin A (IU)',
...
solver = pywraplp.Solver(name='SolveStigler', problem_type=pywraplp.Solver.GLOP_LINEAR_PROGRAMMING)

food = [[]] * len(data)

# Our Objective is to 'minimize the sum of (price-normalized) foods'
...
"""
Amount of money to be spent on food j  = food[j] 
Amount of nutrient(i) provided by food(j) per dollar  =  data[j][i+3]

So, Amount of Nutrient i provided by food j = data[j][i+3] * food[j]

Minimum requirement for nutrient i = nutrients[i][1]
"""

# Create constraints, One per nutrient
